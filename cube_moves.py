...
def horizontal(cube , row , direction ):
    ...
    # side faces shift 
        ...
        # right -> direction == 1 
            ...
            print("ERROR direction specified is not possible ")
            return 
...
def vertical(cube, column , direction ):
    if column < len(cube.struct[0]):
        ...
        print("ERROR column number is out of bound for the structure ")
        return 

def top_side(cube,column , direction):
    if column < len(cube.struct[0]):
        for i in range(len(cube.struct[0])):
            if direction == 0:
                 cube.struct[0][column][i], cube.struct[1][-i-1][column], cube.struct[3][i][-column-1], cube.struct[5][-column-1][-1-i] = (cube.struct[3][i][-column-1],cube.struct[0][column][i],cube.struct[5][-column-1][-1-i],cube.struct[1][-i-1][column])
            elif direction == 1: #Twist up
                cube.struct[0][column][i], cube.struct[1][-i-1][column], cube.struct[3][i][-column-1], cube.struct[5][-column-1][-1-i] = (cube.struct[1][-i-1][column],cube.struct[5][-column-1][-1-i],cube.struct[0][column][i],cube.struct[3][i][-column-1])
            else:
            ...
            if column == 0 :
                if direction == 0:
                    cube.struct[4] = [list(x) for x in zip(*reversed(cube.struct[4]))]
                elif direction == 1:
                    cube.struct[4] = [list(x) for x in zip(*cube.struct[4])][::-1]
            elif column == len(cube.struct[0])-1:
                if direction == 0:
                    cube.struct[1] = [list(x) for x in zip(*reversed(cube.struct[1]))]
                elif direction == 1:
        ...
        print("Error column not in the structure ")